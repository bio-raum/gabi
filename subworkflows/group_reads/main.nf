...
workflow GROUP_READS {
    ...
    */
    ...
    illumina = reads.filter {m,_r -> m.platform == "ILLUMINA"}
    ont = reads.filter { m,_r -> m.platform == "NANOPORE"}
    pacbio = reads.filter { m,_r -> m.platform == "PACBIO"}
    ...
    */
    ...
    ch_pb_reads_cross_grouped_joined_filtered   = ch_pb_reads_cross_grouped_joined.filter { it -> !(it.last()) } // remove those entries that had no joined short reads
    ch_pb_reads_only                            = ch_pb_reads_cross_grouped_joined_filtered.transpose().map { it -> [ it[1], it[2]] }
    ...
    ch_pb_reads_cross_grouped_joined_filtered      = ch_pb_reads_cross_grouped_joined.filter { it -> !(it.last()) } // [ sample_id, [meta, ill], null ]

    // Channel with pacbio reads and optional illumina reads for polishing
    // [ meta, [ illumina], pacbio ]

    ch_reads_with_pacbio                        = ch_pb_reads_cross_grouped_joined.filter { it -> it.last() } // [ sample_id, [meta, ill], [meta, pb]]
    ch_reads_with_pacbio_no_short               = ch_reads_with_pacbio.filter { it -> !it[1] }
    ch_reads_with_pacbio.filter { it -> it[1] }.transpose().map { it -> [ it[3], it[2], it[4] ] }.map { m, i, n ->
        def newMeta = [:]
    ...
    */
    ...
    ch_ont_reads_cross_grouped_joined_filtered  = ch_ont_reads_cross_grouped_joined.filter { it -> !(it.last()) } // remove those entries that had no joined short reads
    ...
    ch_reads_with_nanopore                      = ch_ont_reads_cross_grouped_joined.filter { it -> it.last() }
    ...
    ch_dragonflye                               = ch_dragonflye.mix(ch_reads_with_nanopore_no_short.transpose().map { it -> [ it[2], [], it[3]] })

    // Samples for which we only have short reads
    ch_short_reads_only                         = ch_ont_reads_cross_grouped_joined_filtered.mix(ch_pb_reads_cross_grouped_joined_filtered).transpose().map { it -> [ it[1], it[2]] }.unique()
    ...
    */
    ...
    ch_short_reads_only_final                   = ch_short_reads_with_paired_long_reads.filter { it -> !it[-2] & !it[-1]}.map { it -> [ it[1], it[2]]}
    ...
    pacbio_hybrid   = ch_reads_with_pacbio_no_short.transpose().map { it -> [ it[2], [], it[3]] }.mix(ch_reads_with_pacbio_and_short)
