...
This is a very short list of steps required to get your started with GABI. Please see our complete [installation](installation.md) and [usage](usage.md) guides to answer any questions you are left with after reading this. 
...
!!! info Nextflow arguments and pipeline options
    ...
    Those that start with a single dash (`-profile`, `-r`) are Nextflow arguments, and are documented in the [Nextflow documentation](https://www.nextflow.io/docs/latest/cli.html). Those starting with a double dash (`--build_reference`, `--run_name`) are GABI options and are detailled in the [usage section](usage.md)
